...
description: Connect remote MCP servers to RevoEngine Agents with synchronized manifests, Secret bindings, and per-tool governance.
icon: plug-circle-plus
---

An MCP server plugin lets an Interactive or Autonomous Agent use tools from an approved remote MCP server. The plugin is an administrative integration boundary: connecting a server does not automatically expose every remote capability to every AI workflow.
...
### Immediate versus deferred tools

Use immediate loading for a small, central toolset whose schemas are required in most tasks using the plugin. Use deferred loading when a server exposes many tools or when a capability is relevant only to a narrow workflow.

Deferred does not mean disabled. RevoEngine keeps a compact reviewed summary available for capability selection, then loads the exact admitted operation when the task needs it. This reduces model context without hiding the integration entirely.

Auto-discovery follows the same pattern: an active plugin can become a deferred candidate across eligible Agent work, but its full tool catalogue is not injected into every request and it cannot execute until current attachment, policy, identity, and operation checks succeed.
