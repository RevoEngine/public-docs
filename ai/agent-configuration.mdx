...
Choose the Agent's default permission level in the **Tools** section:

| Permission | Behavior |
| --- | --- |
| **Restricted** | Read-only work and actions explicitly permitted without approval can proceed. Approval-sensitive changes, side-effecting code, and external actions wait for the user. |
| **Auto Review** | When an action would otherwise need approval, RevoEngine checks it against the user's explicit request and available authorized context. Clearly matching actions can continue automatically; uncertain or higher-risk actions wait for the user. |
| **Full Access** | Normal per-action approval prompts are skipped. Service-account permissions, tenant rules, target access, plugin restrictions, and validation still apply. |

Capability-specific overrides can narrow or expand the level for a selected capability. Enabling a capability is not an access grant: execution still depends on the service account, instance policy, target access, and the capability's own restrictions.
...
| `scheduleFor` | No | ISO timestamp with timezone or Unix milliseconds, from request acceptance through 30 days ahead. The item is recorded immediately and becomes claimable at this time. |
...
- `scheduleFor` as an ISO timestamp with timezone or Unix milliseconds, up to 30 days ahead;
...
A delayed direct run is visible immediately as `CREATED`, with `scheduledFor` in its details. It does not occupy an execution slot while waiting for its initial time. Once due, it must pass the Agent's concurrency gate; waiting for capacity is `PENDING` with a capacity wait reason. Later continuation wake times are separate from the original accepted schedule.

<Warning>
  Use `scheduleFor` for both Inbox and direct runs. A past public timestamp or one more than 30 days ahead is rejected. An already accepted request that reaches dispatch after its time becomes immediately eligible; eligibility is not a promise of an exact start time.
</Warning>
