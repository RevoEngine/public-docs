...
Auto Review is designed to preserve the user's intent while reducing routine approval prompts:
...
<Info>
  Auto Review does not expand access or replace least-privilege configuration. It adds a review boundary between a clear user request and the action proposed to carry it out.
</Info>
