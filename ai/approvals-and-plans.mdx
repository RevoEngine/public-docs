...
| Permission policy | Which side effects require approval? | **Restricted** for maximum supervision or **Auto Review** for governed autonomy |
...
**RevoShield-1** is RevoEngine's specialized Auto Review model. It is designed to preserve the user's intent while reducing routine approval prompts. Before an eligible action proceeds, RevoShield-1 evaluates the exact proposed effect against the user's request, the authorized context available to the task, and the evidence required for that operation.
