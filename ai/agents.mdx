...
<Steps>
  ...
  <Step title="Configure runtime behavior">
    Choose the model, reasoning effort, execution mode, planning policy, and persistent Agent instructions. GPT-5.6 Sol, GPT-5.6 Terra, and GPT-5.6 Luna are currently supported; autonomous runs use Standard processing. Set concurrency and run limits for the expected workload. See the [current model support table](/ai/assistant#current-model-support) for the shared Assistant and Agent matrix.
  </Step>
