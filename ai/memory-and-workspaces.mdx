...
The workspace root is the default destination for Agent-generated files and is protected from mutation. Workspace-scoped operations stay within that root. Explicitly selected files elsewhere remain available through normal Storage access checks; reading or attaching such a file requires permission from the execution identity. Interactive Assistants create new files in Private by default.
