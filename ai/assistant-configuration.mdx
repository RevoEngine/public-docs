...
The composer can override model, reasoning, planner behavior, or plugin selection for the current work. A per-turn choice changes that execution context without rewriting the saved account default. Work mode, persona, and custom instructions remain account-level response preferences until changed in **Settings → Assistant**.

For example, keep **Medium + Standard + Balanced** as a daily default, then select **High + Review required + Coding** for a risky cross-Component migration.
