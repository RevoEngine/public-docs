...
The public Files contract does not provide move, archive, restore, ACL, retention, or multipart-session operations. Do not build a recovery promise around compatibility controls that are not part of that contract.
...
See [Storage](/operate/storage) for the complete lifecycle, [HTTP integrations](/low-code/http) for streamed migration, and the generated [Storage method reference](/low-code/reference/storage) for current signatures only.
