...
`componentVersion` pins a known low-code version for a controlled release. Omit it only when the task should intentionally resolve the latest active version; make that choice explicitly for a new Job Template. When repairing its Component, preserve each existing Template binding unless the operator explicitly approves a migration. `triggerUser` is the principal whose roles and resource ACLs apply at execution time; choose a dedicated service account for unattended work.
