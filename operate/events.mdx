...
| React to a domain signal | `CUSTOM` with `customType` such as `ORDER_APPROVED` |
...
| **Run** | Custom definitions only: publishes a message under the stored type. Native lifecycle events are replayed from Event History. |
...
Choose **Type → Custom** when the signal belongs to your application rather than a RevoEngine resource lifecycle. In the **Custom type** field enter the event type, for example `ORDER_APPROVED`. The API stores `type: "ORDER_APPROVED", custom: true`; no prefix is added.

Use a stable namespace and past-tense business fact. A custom event should announce something that already happened, not hide a command such as `SEND_EMAIL_NOW`.
...
```json
{
  "or": [
    ...
    {
      "and": [
        { "field": "type", "op": "eq", "value": "ORDER_APPROVED" },
...
Saved active custom Events expose **Run** to `AUTOMATION_RUNNER` and `AUTOMATION_ADMIN`.
...
The `custom` boolean is the only classifier. `custom: true` permits a dynamic business message; `custom: false` selects a supported platform lifecycle event and can only be replayed from existing history. Matching uses both `type` and `custom`, so identical names in different classes do not collide. The UI's Custom selector is only a presentation control.
...
| `STORAGE_DELETED` | The archived entry entered permanent deletion and managed content cleanup. | Remove external references and retention evidence. |
...
See the full [file-processing workflow](/platform/file-processing-workflow) for upload, schema, finalization, and retry-safe output design.
...
<AccordionGroup>
  ...
  <Accordion title="A custom event matches too many definitions">
    Use a more specific business event type or add message/metadata filters. Keep each definition responsible for one target so unwanted fan-out can be disabled independently.
  </Accordion>
</AccordionGroup>

## Production checklist

- Give custom types stable names and set `custom: true`.
