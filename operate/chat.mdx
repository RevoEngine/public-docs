...
## Files in notification emails

Eligible notification emails include download cards for attached Storage files, with the file type, size, expiration time and a download action. Choose a link validity from **one to seven days**. Email links expire independently of the file's retention policy. Treat a download link as sensitive access information and avoid forwarding it beyond the intended recipients.

System messages, unread Chat digests and Job notifications share the same header, workspace actions and preference footer, with different content for each message type. If an attachment link cannot be created, the delivery is retried before sending instead of silently removing its card.

Chat persistence, provider acceptance and confirmed email delivery are separate states. If the provider outcome is uncertain, reconcile it before retrying a send to avoid duplicates.
...
## Email preferences and unread digests

In **Settings → Notifications**, the chat and system-message email preference
controls email for these conversations. It is enabled by default and can be
turned off independently of the administrator's job-failure notification choices.

System messages can produce an immediate email. Ordinary unread Chat messages
can be collected into a digest; read messages and muted or archived conversations
are excluded. Each unread receipt can enter at most one digest, and eligibility
is checked again before sending. Reading a message in the app does not retract
an email already accepted by the mail provider.

Job-failure email is controlled separately by the per-instance administrator
settings. A job alert sent to the system conversation does not produce an extra
system-message email on top of the dedicated job email. Transactional email is
also separate from the chat preference.

Delivery is tracked persistently and transient failures can be retried. Provider
acceptance means the provider accepted the request; it does not confirm inbox
delivery.
