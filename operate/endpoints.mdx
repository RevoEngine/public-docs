...
Use a Component-backed Endpoint for business logic, database access, integrations, and dynamic responses. Before creating one, decide whether it follows the latest active low-code Component version or pins a deliberately selected version. This is a compatibility decision, not an implementation detail.
...
- prefer one named visible element result with `popResult` for an Endpoint that returns a body;
- return the Component result map only when callers explicitly need multiple visible results;
...
When a Component is repaired, inspect its Endpoint, Job Template, and Agent Tool dependencies. A latest-bound consumer follows the repaired Component and needs response-contract validation; a pinned consumer remains on its recorded version. Do not migrate or bump existing consumers without an explicit compatibility decision from the operator.
